 ...
 *
 * == Fri 5 Aug 2016
 * -- ADDED: preg_match_link - take a markup link and returns the a href and the anchor text
 *
 * == Mon 11 Aptil 2016
 * -- ADDED: ez_lvc() - lvc = load view controller
 * -- CHANGED: Renamed
 *
 * ==  Wed 3 June 2015
 * --- ADDED: ez_get_post_thumbnail_id()
 * --- CHANGED: ez_wp_query() - key for 'ez' args from (just) 'ez' to 'ez_args' to make add clarity
 *
 ...
 *
 * -- Wed 31 Dec 2014 - Added: ez_wp_get_attachment_image - TODO
 *
...
// == End: Boilerplate
	...
	static public function preg_match_link($str_link = ''){

		preg_match_all("|<a.*(?=href=\"([^\"]*)\")[^>]*>([^<]*)</a>|i", $str_link, $arr_link);

		$obj_ret = new \stdClass();
		$obj_ret->href = '123';
		$obj_ret->anchor = '';
		if ( isset($arr_link[1][0]) && filter_var($arr_link[1][0], FILTER_VALIDATE_URL) && isset($arr_link[2][0]) ){
			$obj_ret->href = $arr_link[1][0];
			$obj_ret->anchor = $arr_link[2][0];
		}
		return $obj_ret;
	}
        ...
        /**
         * TODO = REMOVE? - Given a list of arg will create an add_image_size args object
         *
         * TODO Method detailed description
         *
         * @author Mark Simchock <mark.simchock@alchemyunited.com>
         *
         * @param    bool_active      is this image size active
         * @param    string $str_name name of the image size
         * @param    int $width
         *                            Indent to the description's starting point
         *                            for long ones.
         *
         * @return obj the args in come out as an obj
         *
         */
        static public function ez_ais_obj(
            $bool_active = true,
            $str_name = '',
            $mix_width = 500,
            $mix_height = false,
            ...
            $int_offset = 0,
            $bool_nc_active = true,
            $str_nc_select = 'Ratio: Square',
            $bool_pf_active = true,
            $str_pf_w = 'w',
            $arr_pf_exclude = array()){
         ...
         */
            ...
            $obj->name = $str_name;
            $obj->width = $mix_width;
            $obj->height = $mix_height;
            $obj->crop = $bool_crop;
            $obj->ratio = $str_ratio;
            $obj->orientation = $str_orientation;
            $obj->offset = $int_offset;

            $obj_nc->active = $bool_nc_active;
            $obj_nc->select = $str_nc_select;
            $obj->names_choose = $obj_nc;

            $obj_pf->active = $bool_pf_active;
            $obj_pf->w = $str_pf_w;
            $obj_pf->exclude_from_sizes = $arr_pf_exclude;
         ...
         * WP_Query gets The ezWay treatment
         *
         * The general idea here is to centralize your "data grathering" prior to displaying it. this allows vou to focus (mostly) on presentation.
         *
         * @author Mark Simchock <mark.simchock@alchemyunited.com>
         *
         * @param    array $arg1                                                                  key = 'ez' - an assoc array  with additional ez-centric args
         *                                                                                        - 'get_post_meta'            => bool,    // default: true
         *                                                                                        // false: skip this step
         *                                                                                        - 'get_post_thumbnail_id'   => bool,    // default: true
         *                                                                                        // false: skip this step
         *                                                                                        *                                            - 'get_post_class'          => bool,    // default: true
         *                                                                                        // false: skip this step
         *                                                                                        - 'taxonomies'                => mixed,   // - default: array filled by get_object_taxonomies('post_type')
         *                                                                                        // - true: array filled by get_object_taxonomies('post_type').
         *                                                                                        // - false: skip this step
        ...
        static public function ez_wp_query( $arr_args = array() )
        {
            ...
            if ( isset($arr_args['ez_args']) && is_array( $arr_args['ez_args'] ) && !empty($arr_args['ez_args']) ) {

                $arr_args_ez = $arr_args['ez_args'];
                unset($arr_args['ez_args']);
            }
            ...
            // default for get_post_thumbnail_id is true. this is, if we're using ez_wp_query then we always want the post's get_post_thumbnail_id unless we specify otherwise.
            if ( !isset($arr_args_ez['get_post_thumbnail_id']) || (isset($arr_args_ez['get_post_thumbnail_id']) && $arr_args_ez['get_post_thumbnail_id'] !== false) ) {
                $wp_query->posts = self::ez_get_post_thumbnail_id( $wp_query->posts );
            }


            // default for ez_get_post_class is true. this is, if we're using ez_wp_query then we always want the post's ez_get_post_class unless we specify otherwise.
        ...
        /**
         * Similar to get_post_thumbnail_id() but loops over the whole $posts object
         *
         * TODO - comments
         */
        ...
        /*
         * lvc = load view controller
         */
        static public function ez_lvc( $obj_vc_args = '', $obj_all = '')
        {

        	// var_dump($obj_vc_args);

            if ( ! is_object( $obj_vc_args ) ) {
                return;
            }

            $obj_defaults = new \stdClass();

	        //
	        $obj_defaults->active = true;
	        $obj_defaults->debug = false;

	        // controller
	        $obj_defaults->c_slug = false;
	        $obj_defaults->c_name = null;
	        $obj_defaults->c_class = '';
	        $obj_defaults->c_args = new \stdClass;

	        // view
            $obj_defaults->v_slug = false;
	        $obj_defaults->v_name = null;
            $obj_defaults->v_class = '';
            $obj_defaults->v_meth_lang = '';
            $obj_defaults->v_meth_vargs = '';

            // ref: http://stackoverflow.com/questions/455700/what-is-the-best-method-to-merge-two-php-objects
            $obj_vc_args = (object)array_merge( (array)$obj_defaults, (array)$obj_vc_args );

            if ( $obj_vc_args->active === false ){
                return;
            }

            // TODO - remove debug?
            $str_empty = '';
            $str_no_controller_class = '';
            $str_no_view_class = '';
            $str_success = '';
            if ( $obj_vc_args->debug === true ) {
                $str_empty = 'empty || active = false';
                $str_no_controller_class = '! controller_slug || ! controller_class';
                $str_no_view_class = '! view_slug || ! view_class';
                $str_success = 'success';
            }


            if ( ( empty($obj_vc_args->v_slug) && $obj_vc_args->v_slug !== false ) || $obj_vc_args->v_class == '' || ( empty($obj_vc_args->c_slug) && $obj_vc_args->c_slug !== false ) || $obj_vc_args->c_class == '' || $obj_vc_args->active == false ) {

	            return $str_empty;
            }

	        // if the class doesn't exist then include its "template part"
	        if ($obj_vc_args->c_slug !== false) {
		        if ( ! class_exists( $obj_vc_args->c_class ) ) {
			        get_template_part( $obj_vc_args->c_slug, $obj_vc_args->c_name );
		        }
	        }
            
            if ($obj_vc_args->v_slug !== false){
	            if ( ! class_exists($obj_vc_args->v_class) ){
		            get_template_part( $obj_vc_args->v_slug, $obj_vc_args->v_name );

	            }
            }


            // TODO - decide on how to handle a fail
            if ( ! class_exists( $obj_vc_args->c_class ) ) {
                return $str_no_controller_class;
            }
            if ( ! class_exists( $obj_vc_args->v_class ) ) {
	            return $str_no_view_class;
            }

            // get an instance of the controller.
            $obj_c = new $obj_vc_args->c_class( $obj_all, $obj_vc_args->v_meth_lang , $obj_vc_args->v_meth_vargs );

	      // var_dump($obj_c);

            return new $obj_vc_args->v_class( $obj_c );
		 ...
		 * lvc = load view controller
		 */
		static public function ez_lvc2( $obj_vc_args = '', $obj_all = '')
		{
			...
			$obj_v =  new $obj_vc_args->v_class( $obj_c );
         ...
         * TODO - docs
         */
        static public function ez_object_merge( $arr_args ){

            if ( is_array( $arr_args ) ) {
                $arr = array();
                foreach ( $arr_args as $obj ) {
                    if ( is_object( $obj ) && ! empty(obj) ) {
                        $arr = array_merge( $arr, (array) $obj );
                    }
                }
                return (object) $arr;
            }
        ...
        */
        ...
        static public function ez_file_exists( $str_url )
        {
            ...
            if ( strpos( $str_file_headers[0], '404.php Not Found' ) ) {
